...
from src.helixfold.data import pipeline_rna
...
class RNADataPipeline:
  ...
  def __init__(self,
               monomer_data_pipeline: pipeline_rna.RNADataPipeline):
    """Initializes the data pipeline.

    Args:
      monomer_data_pipeline: An instance of pipeline.DataPipeline - that runs
        the data pipeline for the RNA monomer HelixFold system.
    """
  ...
  def _process_single_chain(
      ...
      description: str,
      msa_output_dir: str,
      is_homomer_or_monomer=True) -> pipeline.FeatureDict:
    ...
    with temp_fasta_file(chain_fasta_str) as chain_fasta_path:
      ...
      if not is_homomer_or_monomer:
        rfam_out_path = os.path.join(chain_msa_output_dir, 'rfam_hits.sto')
        if os.path.exists(rfam_out_path):
          all_seq_msa_features = self._all_seq_msa_features(rfam_out_path, query_sequence=sequence)
          chain_features.update(all_seq_msa_features)    

    return chain_features

  def _all_seq_msa_features(self, msa_file, query_sequence):
    """Get MSA features for unclustered uniprot, for pairing."""
    with open(msa_file, 'r') as f:
      msa_lines = f.read()

    msa = parsers.parse_stockholm_RNA(msa_lines, query_sequence)
    # msa = msa.truncate(max_seqs=self._max_uniprot_hits)
    max_align_depth = self._monomer_data_pipeline.total_max_hits
    species_identifer_df = self._monomer_data_pipeline.species_identifer_df
    
    all_seq_features = pipeline_rna.make_msa_features([msa], species_identifer_df, max_align_depth)
  ...
  def process(self,
    ...
    np_example = feature_processing.rna_merge(
...
def process_feat_to_mapping_to_new_token_list(chain_features):

    chain_features['aatype'] = transform_onehot_features(chain_features['aatype'],
                                          old_token_list=residue_constants.rna_nt_types_with_gap,
                                          new_token_id_map=residue_constants.HF3_restype_order,
                                          unmatched_token='N')

    chain_features['msa'] = map_elements_vectorized(chain_features['msa'],
                                        old_token_list=residue_constants.rna_nt_types_with_gap,
                                        new_token_id_map=residue_constants.HF3_restype_order,
                                        unmatched_token='N')

    chain_features['msa_all_seq'] = map_elements_vectorized(chain_features['msa_all_seq'],
                                        old_token_list=residue_constants.rna_nt_types_with_gap,
                                        new_token_id_map=residue_constants.HF3_restype_order,
                                        unmatched_token='N')                                   
...
def transform_onehot_features(onehot_features: np.ndarray, 
    ...
    Args:
        onehot_features (np.ndarray[int]): A set of one-hot encoded features, represented as a 2D integer array.
        old_token_list (List[str]): The old token list, which is a list of strings.
        new_token_id_map (Dict[str]): A dictionary where the keys are old tokens and the values are corresponding new token IDs.
        unmatched_token (str): A string representing the unmatched token. It should have a corresponding entry in the new_token_list.
        
    Returns:
        np.ndarray[int]: The transformed feature set, represented as a 2D integer array.
    '''
    # Ensure the unmatched_token is present in the new_token_id_map
    ...
    transformed_tokens = map_elements_vectorized(token_array, old_token_list, new_token_id_map, unmatched_token)

    # Create an identity matrix of size num_classes
    one_hot_matrix = np.eye(len(new_token_id_map))
    # Use the input array to index the identity matrix
...
def map_elements_vectorized(array, old_token_list: List[str], 
                              ...
                              unmatched_token: str):
    """
    Map each element in a potentially multidimensional np.ndarray[int] to new elements based on a mapping list.
    
    :param array: The original possibly multidimensional np.ndarray[int] array
    :old_token_list (List[str]): The old token list, which is a list of strings.
    :new_token_id_map (Dict[str]): A dictionary where the keys are old tokens and the values are corresponding new token IDs.
    :unmatched_token (str): A string representing the unmatched token. It should have a corresponding entry in the new_token_list.
    :return: The mapped np.ndarray array
    """
    ...
    # Create a function for mapping using the dictionary's get method
    ...
    # Apply the mapping function to the entire array
