...
from src.helixfold.data import mmcif_parsing
from src.helixfold.data import parsers
from src.helixfold.data.tools import kalign
import numpy as np
...
def _process_single_hit(
  ...
  try:
    ...
    # NOTE: hit.sum_probs may be None!
    ...
    warning = ('%s_%s (sum_probs: %s, rank: %s): feature extracting errors: '
               ...
               % (hit_pdb_code, hit_chain_id, sum_probs, hit.index,
...
class HmmsearchHitFeaturizer(TemplateHitFeaturizer):
  ...
  def get_templates(
    ...
    if already_seen:
      ...
      # NOTE: np.object is deprecated, please use object instead.
      template_features = {
