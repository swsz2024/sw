...
def parse_stockholm_RNA(stockholm_string: str, query: Optional[str]) -> Msa:
  """Parses sequences and deletion matrix from stockholm format alignment.
  ...
  if query is not None:
    ...
    if len(name_to_sequence.keys()) == 0:
      ...
      new_name_to_sequence = collections.OrderedDict({'query': query})
...
def remove_empty_columns_from_stockholm_msa_for_hmmer(stockholm_msa: str) -> str:
...
def remove_empty_columns_from_stockholm_msa_for_mmseqs(stockholm_msa: str) -> str:
    """Removes empty columns (dashes-only) from a Stockholm MSA without #=GC RF lines."""
    # First parse all alignment lines
    alignments = collections.OrderedDict()  # preserves order
    for line in stockholm_msa.splitlines():
        line = line.strip()
        if line and not line.startswith(('#', '//')):
            # Handle lines like "seqname ABC-DEF"
            parts = line.split()
            if len(parts) >= 2:  # Some sequences might have spaces in names
                seqname = parts[0]
                alignment = ''.join(parts[1:])
                alignments[seqname] = alignment

    if not alignments:
        return stockholm_msa  # No alignments found, return original

    # Find columns where ALL sequences have '-'
    first_alignment = next(iter(alignments.values()))
    num_cols = len(first_alignment)
    mask = [False] * num_cols  # True means keep column
    ...
    for seqname, alignment in alignments.items():
        processed_alignments[seqname] = ''.join(itertools.compress(alignment, mask))

    # Rebuild the Stockholm file
    output_lines = []
    for line in stockholm_msa.splitlines():
        if line.strip() and not line.startswith(('#', '//')):
            parts = line.split()
            seqname = parts[0]
            if seqname in processed_alignments:
                new_line = f"{seqname} {processed_alignments[seqname]}"
...
def parse_ccd_fasta(ccd_fasta_string: str) -> Sequence[str]:
  """Parses a FASTA string.

  Args:
    fasta_string: A string containing one or more ccd entries.

  Returns:
    A list of strings representing the sequences in the ccd entries.
  """
  pattern_parse_ccd = re.compile(r"\((.{1,10}?)\)")
  matches = pattern_parse_ccd.findall(ccd_fasta_string)
